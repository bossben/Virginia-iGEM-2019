...
model = addMetabolite(model, 'phleth[c]', 'metName', 'Phenylethylene (cytoplasmic)',...
    'metFormula', 'C8H8', 'Charge', 0); 
model = addMetabolite(model, 'phloxi_S2[c]', 'metName', '(2S)-2-Phenyloxirone',...
...
%add new sty reactions. STYDr is already present as ALDD19xr
...
model = addReaction(model, 'STYABr', 'reactionName', 'phenylethylene oxidoreductase',...
    'reactionFormula', 'phleth[c] + fadh2[c] + o2[c] <=> h[c] + fad[c] + h2o[c] + phloxi_S2[c]');
...
%model = addReaction(model, 'STYDr', 'reactionName', 'Phenylacetaldehyde oxidoreductase',...
 % 'reactionFormula', 'pacald[c] + nad[c] + h2o[c] <=> pac[c] + nadh[c] + 2 h[c]');
...
model = changeGeneAssociation(model, 'STYABr', {'StyA', 'StyB'});
model = changeGeneAssociation(model, 'STYCr', 'StyC');
model = changeGeneAssociation(model, 'ALDD19xr', 'StyD');
